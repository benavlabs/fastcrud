...
class BulkOperationResult(BaseModel):
    ...
    items_processed: int = Field(
        ..., ge=0, description="Number of items processed in this batch"
    )
    success: bool = Field(..., description="Whether the batch completed successfully")
    error_message: str | None = Field(
        None, description="Error message if the batch failed"
    )
    error_details: dict[str, Any] | None = Field(
        None, description="Additional error details and context"
    )
    duration_ms: float = Field(
        ..., ge=0, description="Time taken to process this batch in milliseconds"
    )
...
class BulkOperationSummary(BaseModel):
    ...
    operation_type: str = Field(
        ..., description="Type of operation performed (insert, update, delete)"
    )
    total_requested: int = Field(
        ..., ge=0, description="Total number of items requested for processing"
    )
    total_processed: int = Field(
        ..., ge=0, description="Total number of items actually processed"
    )
    successful_count: int = Field(
        ..., ge=0, description="Number of items successfully processed"
    )
    failed_count: int = Field(
        ..., ge=0, description="Number of items that failed processing"
    )
    ...
    success_rate: float = Field(
        ..., ge=0.0, le=1.0, description="Percentage of successful operations"
    )
    average_batch_duration_ms: float = Field(
        ..., ge=0.0, description="Average time per batch in milliseconds"
    )
    items_per_second: float = Field(
        ..., ge=0.0, description="Processing rate in items per second"
    )
    ...
    batch_results: list[BulkOperationResult] = Field(
        default_factory=list, description="Detailed results for each batch"
    )

    # Configuration used
    batch_size: int | None = Field(None, description="Batch size used for processing")
    commit_strategy: str | None = Field(
        None, description="Commit strategy used (e.g., 'batch', 'all')"
    )
    allow_partial_success: bool = Field(
        False, description="Whether partial success was allowed"
    )
...
class BulkInsertSummary(BulkOperationSummary):
    ...
    operation_type: str = Field(
        default="insert", description="Type of operation performed"
    )

    # Insert-specific metrics
    duplicate_count: int = Field(
        default=0, ge=0, description="Number of duplicate records detected"
    )
    constraint_violations: int = Field(
        default=0, ge=0, description="Number of constraint violations"
    )
...
class BulkUpdateSummary(BulkOperationSummary):
    ...
    # Override operation_type with default
    operation_type: str = Field(
        default="update", description="Type of operation performed"
    )

    # Update-specific metrics
    not_found_count: int = Field(
        default=0, ge=0, description="Number of records not found for updating"
    )
    unchanged_count: int = Field(
        default=0, ge=0, description="Number of records that were already up to date"
    )
...
class BulkDeleteSummary(BulkOperationSummary):
    ...
    # Override operation_type with default
    operation_type: str = Field(
        default="delete", description="Type of operation performed"
    )

    # Delete-specific metrics
    not_found_count: int = Field(
        default=0, ge=0, description="Number of records not found for deletion"
    )
    soft_deleted_count: int = Field(
        default=0, ge=0, description="Number of records soft deleted"
    )
    hard_deleted_count: int = Field(
        default=0, ge=0, description="Number of records hard deleted"
    )
